...
const SESSION_DIR = path.join(SESSION_ROOT, 'session');
...
const BOT_LOGO_PATH = path.join(__dirname, 'public', 'logo.png');
...
let currentQR = null; // latest raw QR string from Baileys, refreshed ~every 60s while unpaired
...
app.get('/qr', async (req, res) => {
    if (isConnected) return res.status(404).json({ error: 'Bot is already connected!' });
    if (!currentQR) return res.status(404).json({ error: 'No QR available yet. Try again in a few seconds.' });
    try {
        const buffer = await QRCode.toBuffer(currentQR, { width: 320, margin: 1 });
...
app.get('/code', async (req, res) => {
    ...
    if (isConnected) return res.json({ error: 'Bot is already connected!' });
    if (pairingInProgress) return res.json({ error: 'A pairing request is already in progress. Please wait.' });
    if (!sock) return res.json({ error: 'Bot socket is not ready yet. Try again in a few seconds.' });

    pairingInProgress = true;
    try {
        const cleanNumber = phoneNumber.replace(/[^0-9]/g, '');
        if (!cleanNumber) return res.json({ error: 'Invalid phone number.' });
        if (sock.authState.creds.registered) return res.json({ error: 'Number already registered!' });
...
function buildMenu() {
    ...
    out += `│ ⚡ *Total Commands:* ${printed.size}+\n`;
        ...
        if (connection === 'close') {
            ...
            if (statusCode === DisconnectReason.loggedOut) {
                ...
                if (!wasRegistered) {
                    ...
                    console.log('⏰ Pairing did not complete (code expired or was rejected). Generate a new one from the pairing page.');
                } else {
                    console.log('🔌 Logged out from a previously linked session. Clearing session and restarting pairing.');
                }
                reconnectAttempts = 0;
                fs.rm(SESSION_DIR, { recursive: true, force: true }, () => startMainBot());
                return;
            }

            if (statusCode === DisconnectReason.badSession) {
                console.log('⚠️ Bad session file. Clearing session and restarting.');
                reconnectAttempts = 0;
                fs.rm(SESSION_DIR, { recursive: true, force: true }, () => startMainBot());
                return;
            }
            ...
            if (statusCode === DisconnectReason.connectionReplaced) {
                ...
                console.log('⚠️ Connection replaced by another session. Not auto-reconnecting.');
                return;
            }
            ...
            reconnectAttempts++;
            const delay = Math.min(3000 * (2 ** (reconnectAttempts - 1)), MAX_RECONNECT_DELAY_MS);
            console.log(`🔌 Connection closed. Status: ${statusCode || 'unknown'}. Reconnecting in ${Math.round(delay / 1000)}s (attempt ${reconnectAttempts})...`);

            clearTimeout(reconnectTimer);
            reconnectTimer = setTimeout(startMainBot, delay);
        } else if (connection === 'open') {
            isConnected = true;
            reconnectAttempts = 0; // reset backoff once we're stably connected
            clearTimeout(reconnectTimer);
            console.log(`🤖 🚀 ${BOT_NAME} Power Bot Successfully Connected to WhatsApp! 🔥`);
        }
    });
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    startMainBot().catch((err) => console.log('Failed to start bot:', err));
});
